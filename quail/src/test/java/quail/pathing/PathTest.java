...
class PathTest {
  ...
  void getCurrentPoint() {
    ...
    assertEquals(0, path.getCurrentPointIndex());
    assertEquals(4, path.lastPointIndex);

    // Test that if we set to the second to last point, the current point is the second to last
    // point
    movePathIndexForward(3); // current index: 3
    assertEquals(poseFourth, path.getCurrentPoint());

    // Test that if we set to the last point, the current point is the last point
    path.incrementCurrentPointIndex(); // current index: 4
    assertFalse(
        path.isFinished()); // Make sure we aren't falling into the case the path is finished
    assertEquals(poseEnd, path.getCurrentPoint());

    assertEquals(4, path.getCurrentPointIndex());
    assertFalse(path.isFinished());
    // Test that if we are finished, the current point is null
    path.incrementCurrentPointIndex(); // current index: 5
    assertTrue(path.isFinished()); // we are finished now
    assertThrows(IllegalStateException.class, path::getCurrentPoint);
  }
  ...
  void distanceToCurrentPoint() {
    ...
    // Test that when the path is finished, the distance to the current point throws an illegal
    // state
    movePathIndexForward(5);
    assertThrows(
        IllegalStateException.class, () -> path.distanceToCurrentPoint(new Pose2d(0, 0, 0)));
    assertTrue(path.isFinished());
  }

  @Test
  void vectorLastToCurrentPoint() {
    // Test that if this is the first point, the vector from the last point to the current point is
    // 0, 0
    ...
    // Advance to the end and confirm that once finished, the vector from the last point to the
    // current point is null
    movePathIndexForward(4);
    assertTrue(path.isFinished());
    assertThrows(IllegalStateException.class, path::vectorLastToCurrentPoint);
  }
  ...
  void nearestPoint() {
    ...
    assertEquals(poseSecond, path.nearestPoint(new Pose2d(-1, -1, 0), 1));
  ...
  void remainingLength() {
    ...
    // Check that as we advance the length remaining goes down by 0.5 if we are halfway to the
    // second point
    path.incrementCurrentPointIndex();
    ...
    movePathIndexForward(3);
    assertEquals(1.5, path.remainingLength(new Pose2d(0.0, 0.5, 0)));
  }
}
